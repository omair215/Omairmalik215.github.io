/**
 * Created by omalik on 7/10/2016.
 */
/**
...
var flickerObj = JSON.parse(jsonString); // Convert JSON String into Javascript Object
//console.log(flickerObj) - Test to make sure that this Javascript Object is valid
 ...
 */
for(var i = 0; i < flickerObj["items"].length; i++){ //Iteration for moving over the items array in FlickerObj
    var currentPic = flickerObj["items"][i]; //Counter for moving loop to next picture
    console.log(currentPic["title"] + ", " + currentPic["date_taken"]); //Write down title and date for picture
};
/*
 for (var prop in obj) {
 ...
 for (var items in flickerObj) {
 if( flickerObj.hasOwnProperty( items ) ) {
 console.log("'items'" + prop + " = " + obj[prop]);
 }
 }
for(var i = 0; i< grungeAlbumsJS["albums"].length; i++){
    var currentAlbum = grungeAlbumsJS["albums"][i];
    console.log("Album: %s \n Artist: %s \n Units Sold: %d", currentAlbum["name"])
}
*/
/*
 ...
 */
for(var x = 0; x < flickerObj["items"].length; x++){ //Iteration for moving over the items array in FlickerObj, So that we don't get undefined values, I'm setting it to be less then  or equal to instead of equal to.
        var currentLink = flickerObj["items"][x]; //Counter for moving loop to next Link
        var links = console.log(currentLink["link"]); //Write down links in new array 'links'
 ...
 */
var linksJsonstring = JSON.stringify(links);
console.log(linksJsonstring);
